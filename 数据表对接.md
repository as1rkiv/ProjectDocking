...
##### 资源KEY配额用量记录表
``` sql 
CREATE TABLE `resource_key_quota_used` (
	`id` BIGINT AUTO_INCREMENT COMMENT '记录ID',
	`r_key_quota_id` BIGINT NOT NULL COMMENT '资源KEY配额ID',
	`used_count` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '调用次数',
	`used_date`  DATE NOT NULL COMMENT '调用日期',

	PRIMARY KEY (`id`),
	INDEX (`r_key_quota_id`),
	INDEX (`date_time`)
) ENGINE = InnoDB COMMENT = '资源KEY配额用量记录表';
```


##### 资源KEY服务能力配额关联表

关联表：[[#资源KEY表]]、[[#资源KEY配额表]]、[[#服务能力表]]、[[#服务能力分组表（≈资源计费模版）]]
``` sql
-- 前提条件
-- 1. 录入 **服务能力**
-- 2. 录入 **服务能力分组**
...
--
-- 该表作用
-- 在录入 **资源KEY** 时，前端根据 3 的关联，
-- 供录入人员选择该 **资源KEY** 在平台上真实的不同能力组的计费模式（额度、重置周期），
-- 然后录入 **配额量** 和 **并发量** ，
-- 后端根据前端传回的数据，生成对应数量的 **资源KEY配额**，并将 **资源KEY配额ID** 记录在此表
...
##### 客户KEY配额用量记录表
``` sql 
CREATE TABLE `client_key_quota_used` (
	`id` BIGINT AUTO_INCREMENT COMMENT '记录ID',
	...
	INDEX (`date_time`)
) ENGINE = InnoDB COMMENT = '客户KEY配额用量记录表';
```

##### 客户KEY服务入口配额关联表

关联表：[[#客户KEY表]]、[[#客户KEY配额表]]、[[#服务入口表]]、[[#服务入口分组表（≈客户计费模版）]]
``` sql
-- 前提条件
-- 1. 录入 **服务入口**
-- 2. 录入 **服务入口分组**
...
--
-- 该表作用
-- 在创建新的 **客户KEY** 时，前端根据 3 的关联，
-- 供创建人员选择该 **客户KEY** 不同能力组的计费模式（额度、重置周期），
-- 然后录入 **配额量** 和 **并发量** ，
-- 后端根据前端传回的数据，生成对应数量的 **客户KEY配额**，并将 **客户KEY配额ID** 记录在此表
